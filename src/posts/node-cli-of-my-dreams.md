...
10. [But wait, there's more](#section10)
11. [A couple of other scripts to help](#section11)
12. [One more, err, I mean two more scripts, with feeling](#section12)
...
While it lived in a Google Sheet, I created a set of Google Forms for entering the data required for each type of the four entries. The result was a Google Sheet with 5 tabs: one that contained all of the records, and one for each of the entries generated by their respective forms. And, with the help of some AI, I wrote some Google Apps scripts that would move the data from each of the form tabs to the main tab. I then created a 6th tab that would, when told which was the current issue, would contain the filtered results of records from the main tab that were to be included in the current issue of the 11ty Bundle.

Overall, the system worked ok, but there were some things that it didn't do and some of these caused more pain than others. For example, I had no good way to validate the date format I had entered. So, from time to time, I'd build the site with a bad date. Another example, and this has become increasingly an issue, I would find a post and while I thought I may have seen it before, there wasn't a simple way to find out if it was already in the main tab. It was doable, but it took more time than I wanted. These were not insurmountable issues, but they continued to gnaw at me and I knew there had to be a better way.

On the back end of this, using the Google Sheets API, I was able to retrieve the main tab at build time and format the data into a nice JSON array of objects, each object representing one of the entries.
...
I could just use VS Code and create a snippet for each of the 4 entry types. Sure, I could, but...I wanted more than the kind of regex-based validation that could be incorporated into a snippet.

But wait, there has to be some kind of customizable JSON editor out in the open source world, right? Well, there are, but most of the things I found are generalized editors that rely on JSON schema to provide validation. I really wanted to keep things simple, and that just didn't sound simple...to me, at least.
...
Oh, and there's this thing called Express that also runs on node. I could build a web server that would serve up the JSON file and allow me to edit it. Again, I read docs and watched videos. I even started down this path by sketching the UI on paper and writing the basic HTML and CSS for the editor to bolt on to Express or Electron.

I have to say that I'm not a fan of frameworks that have so much stuff to wade through to do something that I think is pretty darn simple.
...
I had never written any javascript to read from the file system. I thought that should probably be where to start. I'd also need to be able to write the file back out after adding a new entry (a JSON object) to the end of it. I have to say that I thrashed around for a while to get the basics working. I ultimately realized that I was not building some large, scalable system, but a single-purpose sole-use script. I decided to access the file system using synchronous calls, needing just node's fs. Once I could read the file and write it back out, I was ready to start building the script.

<div id="section5"></div>

## 5. So what should this script do?

Having nailed down basic file read and write, I thought that the simplest thing to start with was to generate a JSON object for each type of entry. That would require me to make use of the inquirer package to request the data for each part of an entry. They vary a bit, but each has some common info, such as the issue number of the blog, a title, and a link. For sites and starters, that's all that was required. For posts, I needed more info (author, date, and categories) and for releases, I needed just a date as the additional item.
...
The image you see at the top of this blog post is something that the inquirer package generates with very little code. In fact, it's this simple:
...
Well, that got me started on a multi-day journey. I'm not that skilled in javascript, so I will admit that I used some AI along the way, more as a tutor, asking it questions more than asking it for code. For example, I'm finding it a great alternative to going to MDN to look up how a particular array function works.

<div id="section7"></div>

## 7. One needs clean data to start with

One of the things that I had the Google Apps script do occasionally is make a backup JSON file and store it in my Google Drive. So I downloaded the latest one and realized that it needed some cleaning. As it turns out, there was some extraneous cruft at the end of some of the entries. Fortunately, said cruft was ignored by my build process. But, I wanted to clean it up...nothing a few find/replace operations couldn't handle.

One of the things that I had forgotten about was that the categories field of the post entries were stored as a string of comma-separated values and not an array of strings. I had corrected for this in one of my build filters. It made sense to get rid of that filter and adjust the data accordingly. Again, it wasn't very difficult.

<div id="section8"></div>

## 8. Updating the 11ty Bundle source

One nice thing about the way I used the Google Sheets API was I isolated the fetching of that data to a single file in my _data_ directory. Once fetched and the JSON array has been handed off, subsequent elements do the slicing and dicing, generating the data for the templates that drive the site. So, all I had to do was replace a call to the fetching function with a reference to the JSON file. In other words, the heart transplant was pretty straightforward and the patient survived.

<div id="section9"></div>

## 9 Oh, so you want to edit what you've entered?

Let's just say that the great feeling of getting something new working never does last that long. I started using it and realized that I would occasionally make a mistake, but I could not edit what I had just entered even though it was just displayed in front of me. I'd have to open the file in VS Code, scroll to the end and delete the last entry, then run the script again. I knew that I needed to add an edit feature.

It sounds simple enough. I'd have to generate a prompt after the initial entry and then ask what do to next. I came up with 3 thoughts: (1) I'd want to save the entry and exit, (2) I'd want to save the entry and add another (perhaps of a different type), or (3) I'd want to edit what I had just entered.

Here's what that looks like for one of [Cory Dransfeldt's recent post](https://coryd.dev/posts/2024/integrating-a-somewhat-custom-cms-with-eleventy/) entries:
...
Part of what makes this workflow so awesome is my recent migration from using the [Alfred](https://www.alfredapp.com/) app as a launcher, snippet generator, and all-around workflow supporter to using [Raycast](https://www.raycast.com/). Raycast is a Mac-only utility that does a lot of what Alfred does, and a lot more.

The first time I heard of Raycast was from [Robb Knight's blog](https://rknight.me/). He's written a few posts - [here](https://rknight.me/blog/trying-raycast-part-one/), [here](https://rknight.me/blog/trying-raycast-part-two/), and [here](https://rknight.me/blog/raycast-follow-up/) -- about his thoughts on Raycast. He really does a lot to infect people...and I mean that in a good way.
...
Another simple script is one to check a URL to see if it's already in the database. While it does that check when I make an entry, if I have a sense that I may have seen the post before, it makes no sense to enter several items and then find out that it's a duplicate. With this script, I can do a simple check before spending more time on it. It looks like this:
...
## 12. One more, err, I mean two more scripts, with feeling
